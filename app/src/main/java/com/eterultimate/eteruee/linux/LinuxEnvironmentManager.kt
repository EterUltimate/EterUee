...
private const val DEFAULT_DISTRIBUTION = "arch"
...
private val UBUNTU_ROOTFS_SOURCES = mapOf(
    "arm64-v8a" to RootfsSource(
        url = "${UBUNTU_BASE_RELEASE_URL}ubuntu-base-24.04.4-base-arm64.tar.gz",
        fileName = "ubuntu-base-24.04.4-base-arm64.tar.gz",
        compression = RootfsCompression.Gzip,
        stripFirstPathSegment = false,
    ),
    "armeabi-v7a" to RootfsSource(
        url = "${UBUNTU_BASE_RELEASE_URL}ubuntu-base-24.04.4-base-armhf.tar.gz",
        fileName = "ubuntu-base-24.04.4-base-armhf.tar.gz",
        compression = RootfsCompression.Gzip,
        stripFirstPathSegment = false,
    ),
    "x86_64" to RootfsSource(
        url = "${UBUNTU_BASE_RELEASE_URL}ubuntu-base-24.04.4-base-amd64.tar.gz",
...
private val LINUX_DISTRIBUTIONS = listOf(
    LinuxDistributionSpec(
        id = "arch",
        aliases = setOf("archlinux", "arch-linux"),
        displayName = "Arch Linux",
        rootfsDirName = "archlinux",
        installScriptName = "install-archlinux.sh",
        executorName = "eteruee-archlinux-proot",
        installExecutorName = "eteruee-archlinux-installer",
        rootfsSources = ARCH_ROOTFS_SOURCES,
    ),
    LinuxDistributionSpec(
        id = "ubuntu",
        aliases = setOf("ubuntu-base", "ubuntu-24.04", "noble"),
        displayName = "Ubuntu 24.04",
        rootfsDirName = "ubuntu",
        installScriptName = "install-ubuntu.sh",
        executorName = "eteruee-ubuntu-proot",
        installExecutorName = "eteruee-ubuntu-installer",
        rootfsSources = UBUNTU_ROOTFS_SOURCES,
    ),
)
...
class LinuxEnvironmentManager(
    ...
    val rootfsDir: File
        get() = rootfsDirFor(distributionFor(DEFAULT_DISTRIBUTION))
    ...
    val installScript: File
        get() = installScriptFor(distributionFor(DEFAULT_DISTRIBUTION))

    fun getStatus(distribution: String = DEFAULT_DISTRIBUTION): LinuxEnvironmentStatus {
        val spec = distributionFor(distribution)
        val primaryAbi = primaryAbi()
        val rootfsSource = rootfsSourceForAbi(spec, primaryAbi)
        val termuxPackages = termuxPackagesForAbi(primaryAbi)
        val rootfsDir = rootfsDirFor(spec)
        val installScript = installScriptFor(spec)
        val rootfsInstalled = isRootfsInstalled(spec)
        val prootExecutable = prootBinary.canExecute()
        val shellRunner = resolveRunner(spec)
        return LinuxEnvironmentStatus(
            ...
            message = when {
                shellRunner != null -> "${spec.displayName} runner is ready"
                rootfsSource == null -> "Unsupported Android ABI for ${spec.displayName} bootstrap: $primaryAbi"
                !rootfsInstalled -> "${spec.displayName} rootfs is not installed"
                !prootBinary.exists() -> "proot binary is missing"
                !prootExecutable -> "proot binary is not executable"
                else -> "${spec.displayName} runner is not ready"
            },
        )
    }

    suspend fun prepareInstallerScript(distribution: String = DEFAULT_DISTRIBUTION): LinuxEnvironmentStatus =
        ...
        downloadDir.mkdirs()
        val installScript = installScriptFor(spec)
        installScript.writeText(buildInstallScript(spec), Charsets.UTF_8)
        installScript.setExecutable(true, false)
        getStatus(spec.id).copy(message = "${spec.displayName} installer script prepared")
    }

    suspend fun install(
        distribution: String = DEFAULT_DISTRIBUTION,
        timeoutSeconds: Int = MAX_TIMEOUT_SECONDS,
        ...
        prepareInstallerScript(spec.id)

        val initialStatus = getStatus(spec.id)
        ...
        try {
            ...
            val source = requireNotNull(rootfsSourceForAbi(spec, primaryAbi()))
            val archive = downloadRootfsIfNeeded(source, installLog)
            val rootfsDir = rootfsDirFor(spec)
            if (!isRootfsInstalled(spec)) {
                extractRootfs(spec, source, archive, installLog)
            } else {
                installLog.appendLine("Rootfs already installed at ${rootfsDir.absolutePath}")
            }
        } catch (error: Throwable) {
            return@withContext getStatus(spec.id).toInstallResult(
                command = "linux.install",
        ...
        val verify = execute(
            distribution = spec.id,
            command = "printf '${spec.displayName} ready: '; head -n 1 /etc/os-release",
            timeoutSeconds = timeout,
        )
        verify.copy(
            stdout = installLog.append(verify.stdout).toString(),
            executor = spec.installExecutorName,
            command = "linux.install",
        )
    }
    ...
    suspend fun execute(
        ...
        val runner = resolveRunner(spec)
            ?: return@withContext LinuxCommandResult(
                stdout = "",
                stderr = getStatus(spec.id).message,
                exitCode = -1,
    ...
    private fun extractRootfs(
        spec: LinuxDistributionSpec,
        source: RootfsSource,
        archive: File,
        log: StringBuilder,
    ) {
        val canonicalRoot = rootfsDirFor(spec).canonicalFile
        ...
        if (!isRootfsInstalled(spec)) {
            error("${spec.displayName} rootfs extraction finished but etc/os-release was not found")
        }
    ...
    private fun resolveRunner(spec: LinuxDistributionSpec): String? =
        prootBinary
            .takeIf { isRootfsInstalled(spec) && it.isFile && it.canExecute() }
            ?.absolutePath
    ...
    private fun isRootfsInstalled(spec: LinuxDistributionSpec): Boolean =
        rootfsDirFor(spec).resolve("etc/os-release").isFile

    private fun primaryAbi(): String =
        Build.SUPPORTED_ABIS.firstOrNull().orEmpty()

    private fun rootfsSourceForAbi(spec: LinuxDistributionSpec, abi: String): RootfsSource? =
        spec.rootfsSources[abi.lowercase(Locale.ROOT)]
    ...
    private fun rootfsDirFor(spec: LinuxDistributionSpec): File =
        File(baseDir, spec.rootfsDirName)

    private fun installScriptFor(spec: LinuxDistributionSpec): File =
        File(baseDir, spec.installScriptName)

    private fun distributionFor(value: String?): LinuxDistributionSpec {
        val normalized = value
            ?.trim()
            ?.lowercase(Locale.ROOT)
            ?.takeIf { it.isNotBlank() }
            ?: DEFAULT_DISTRIBUTION
        return LINUX_DISTRIBUTIONS.firstOrNull { spec ->
            spec.id == normalized || normalized in spec.aliases
        } ?: error(
            "Unsupported Linux distribution: $value. Supported: ${
                LINUX_DISTRIBUTIONS.joinToString(", ") { it.id }
            }"
        )
    }

    private fun buildInstallScript(spec: LinuxDistributionSpec): String {
        val primaryAbi = primaryAbi()
        val rootfsSource = rootfsSourceForAbi(spec, primaryAbi)
        val rootfsDir = rootfsDirFor(spec)
        return """
            ...
            cat <<'EOF'
            EterUee installs ${spec.displayName} from app code, not this shell script.
...
private fun LinuxEnvironmentStatus.toInstallResult(
    ...
    stderr = stderr,
    exitCode = exitCode,
    executor = LINUX_DISTRIBUTIONS.firstOrNull { it.id == distribution }?.installExecutorName
        ?: "eteruee-linux-installer",
    shell = "kotlin",
...
private data class LinuxDistributionSpec(
    val id: String,
    val aliases: Set<String>,
    val displayName: String,
    val rootfsDirName: String,
    val installScriptName: String,
    val executorName: String,
    val installExecutorName: String,
    val rootfsSources: Map<String, RootfsSource>,
)
...
data class LinuxEnvironmentStatus(
    val distribution: String = DEFAULT_DISTRIBUTION,
    val distributionName: String = "Arch Linux",
